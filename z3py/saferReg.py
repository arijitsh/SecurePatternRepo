...
for pattern in set(patternList):
    ...
    index = 0
    maxOnTime = 0    
    innerCircleDepth=1 #new
    maxIteration = settlingTime+5 #new
    isSat = 1
    while innerCircleDepth > 0 and isSat==1:#new      
        ...
        for varcount in range(1,x_count+1):
            f.write("s.add(Or(x"+str(varcount)+"_0 <= ("+str(innerCircleDepth)+"*"+str(outerCircle[varcount-1])+"),x"+str(varcount)+"_0 >= ("+str(innerCircleDepth)+"*"+str(-outerCircle[varcount-1])+")))\n")
            f.write("s.add(Or(z"+str(varcount)+"_0 <= ("+str(innerCircleDepth)+"*"+str(outerCircle[varcount-1])+"),z"+str(varcount)+"_0 >= ("+str(innerCircleDepth)+"*"+str(-outerCircle[varcount-1])+")))\n")
        ...
        for i in range(K):
            ...
            for varcount1 in range(1,y_count+1):
                ...
                # forallvars+="attackOnY"+str(varcount1)+"_"+str(i)+" = Real('attackOnY"+str(varcount1)+"_"+str(i)+"'),"
        ...
        for i in range(K):
            for varcount in range(1,x_count+1):
                expr_inclusion+="xabs"+str(varcount)+"_"+str(i+1)+" >"+str(outerCircle[varcount-1])+"," #new
        ...
        if isSat==0:
            ...
            innerCircleDepth= innerCircleDepth-0.05 #new
            # K=K+1 #new                           
