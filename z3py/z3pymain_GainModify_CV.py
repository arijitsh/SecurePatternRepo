    ...
    for count in range(len(attackPoints)):
        ...
        while attackLen<=counter and isSat == 0:
            ...
            while (index + attackLen)<=end and (isSat == 0):
                ...
                fileName1 = dt + "_GainModify_" +str(start)+ "_"+str(attackLen)+"_WithRange"+str(attackRange)
                ...
                for i in range(counter):
                    expr_safety+="e1_"+str(i+1)+"<-"+str(safety)+",e2_"+str(i+1)+"<-"+str(safety)+","
