...
for pattern in set(patternList):
    ...
    f0 = open(path+modelName+"_withY0.z3result", "w+")
    f0.write("0")
    f0.close()
    ...
    while isSat == 0:  
        ...
        while (index<patternLen) and (isSat == 0):
            ...
            print(dropPattern)
            print("\n")
            fileName = modelName + "_withY0_" + str(th) + "_" +str(index)+"_"+str(attackLen)+"_"+str(K)+"_"+str(pattern)+".py"
            ...
            f.write("\tprint(s.check())\n")
            f.write("\tisSat = 1\n")
            f.write("\tf0 = open(\""+path+modelName+"_withY0.z3result\", \"w+\")\n")
            f.write("\tf0.write(\"1\")\n")
            f.write("\tf0.close()\n") 
            f.write("\tm = s.model()\n")
            ...
            f0 = open(path+modelName+"_withY0.z3result", "r")
            if f0.mode == 'r':
                content = f0.read()
                isSat = int(content)
            f0.close()
    ...
    final = open(path+modelName+"_withY0_final_downtime.result", "a+")
