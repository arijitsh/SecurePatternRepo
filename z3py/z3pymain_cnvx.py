...
isDelayed = 0
yAffected = 0
filesuffix= "_combinedTest_"
# firstline="innerCircleDepth="+innerCircleDepth+""
if isDelayed == 1:
    filesuffix=filesuffix+"_uDelayed_"
if yAffected == 1:
    filesuffix=filesuffix+"_yAffected_"
else:
    filesuffix=filesuffix+"_OnlyUAltered_"
####################################################################
...
for pattern in set(patternList):
    ...
    while isSat == 0:  
        ...
        while (index<patternLen) and (isSat == 0):
            ...
            fileName = modelName + filesuffix + str(innerCircleDepth)+"_" + str(th) + "_"+str(attackLen)+"_"+str(pattern)+".py"
            f = open(path+fileName, "w+")
            f.write("#U altered, Y altered, U delayed*\n")
            f.write("from z3 import *\n")
            f.write("import math\n")
            f.write("import timing\n")
            ...
            for varcount in range(1,x_count+1):
                ...
                f.write("s.add(Or(x"+str(varcount)+"_0 == "+str(initialRange[varcount-1,0])+","+"x"+str(varcount)+"_0 == "+str(initialRange[varcount-1,1])+"))\n")
