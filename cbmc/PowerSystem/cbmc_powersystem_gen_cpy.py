...
u_attack = np.zeros(shape=(u_count,attackLen),dtype=float)
y_attack = np.zeros(shape=(y_count,attackLen),dtype=float)
    ...
    for counter in range(K):
        ...
        for varcount1 in range(u_count):
            if index+startpoint == counter :
                f.write("\t\tu_attack["+str(varcount1)+"]["+str(counter)+"] = AttackFormat2(syn);\n")
            else:
                f.write("\t\tu_attack["+str(varcount1)+"]["+str(counter)+"] = 0;\n")
        for varcount1 in range(y_count):
            if index+startpoint == counter and varcount1 == 0:
